...
Click the **Requests** button in the navigator.

Select the **Priorities** tab.

Click the **Add** button. A new priority opens to the right.

Enter a descriptive name for the priority (replace the text "Priority name" above the tabs).
...
Under **Time frame** you specify the time frame that applies to request processing.
...
In the **Escalating events** tab, you specify how the escalation chain operates in response to different events in the request. The following options are available:
...
Click **Save**. The priority is created. Now you can, for example, link escalation levels to this priority. See [Create escalation levels](admin.listPriorities.editAlert.newAlert.md)
