---
uid: help-en-admin-listpriorities-listpriorities-deletepriority
title: admin listPriorities listPriorities deletePriority
description: admin listPriorities listPriorities deletePriority
author: SuperOffice RnD
so.date: 06.29.2022
keywords: Settings and maintenance
so.topic: help
language: en
---

# Delete priorities

You cannot delete priorities, but you can set the status of a priority to **Deleted**, to make it unavailable to users and customers. [!include[To do this](../../includes/to-do-this.md)]

1. Click the **Requests** button in the navigator.
2. Select the **Priorities** tab.
3. Click the required priority. The **Priority** tab appears.
4. Click the **Deleted** check box.
5. Click **Save**. The priority will then no longer be available, for example, when users create new requests.

> [!TIP]
> To show deleted/inactive priorities, click the **Show deleted items** check box below the list of priorities.
