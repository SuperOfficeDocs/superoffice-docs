...
When you add an item to the **General - Country** list in Settings and maintenance, the **Edit list item** dialog is not the same as described under [Add items](Adding_items.md).
...
5. For each of the next three fields, click the arrow ( ![icon](../media/pil_ned.bmp) ) and select the required address format from the list. The same format will usually apply for all three fields, but, for some countries, it may be useful to vary the formats, depending on what you are using the address for:
...
7. Click the arrow ( ![icon](../media/pil_ned.bmp) ) next to the **Currency** field and select a currency from the list.
