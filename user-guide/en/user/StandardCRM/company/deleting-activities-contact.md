---
uid: help-en-deleting-activities-contact
title: Deleting activities Contact
description: Deleting activities Contact
author: SuperOffice RnD
so.date: 06.29.2022
keywords: CRM
so.topic: help
language: en
---

# Delete activities in the Company screen

To delete an activity in the **Activities** section tab:

1. Go to the required company in the **Company** screen. (See [Use the Find screen](../key-functions/Find-dialog.md).)
2. Select the **Activities** section tab.
3. Select the required activity.
4. Click **Delete** at the bottom left under the section tabs.
5. When asked if you really want to delete the activity, click **Yes**. (For more details, see [Delete items in SuperOffice CRM](../key-functions/Deleting-elements-in-CRM.md).)
