---
uid: help-en-copy-companies
title: Copy companies
description: Copy companies
author: SuperOffice RnD
so.date: 06.29.2022
keywords: CRM
so.topic: help
language: en
---

# Copy companies

[!include[Requirement](../includes/note-req-manage-entities.md)]

At some stage, you may want to copy an existing company to a new company. For example: an existing company has set up a new branch. The new branch will use mostly the same company information as the existing one.

To copy the company:

1. [!include[Go to company](../includes/goto-company.md)]

2. Select **Manage companies** from the ![icon][img1] **Task** button, and click **Copy company**.
    The **Copy company** dialog opens. The source company is specified under **From**.

    <!-- Fix reuse ID=a1 -->

    ![Copy company dialog -screenshot][img3]

3. Enter the name of the new company and, where appropriate, a department under **To**.

4. Under **Copy contacts with all information**, you can choose from:

    * **Do not copy**. If you select this option, the company details are copied, but not the contacts.
    * **Copy**. If you select this option, the company details and the contacts are copied.

    > [!NOTE]
    > If you want to see the results before you copy the company, click **Show results**: ![icon][img2]

5. Click **Copy**. The new company is created with the specified information, and is given its own number and code. You can then edit the company information if required (see [Edit companies][2]).

> [!NOTE]
> Activities, sales, interests, project membership and selection membership are not copied.

<!-- Referenced links -->
[1]: ../standardcrm/key-functions/find-dialog.md
[2]: editing-companies.md

<!-- Referenced images -->
[img1]: ../../../media/icons/btn-menu.png
[img2]: ../../../media/icons/vis-resultat.bmp
[img3]: media/copy-company.bmp
