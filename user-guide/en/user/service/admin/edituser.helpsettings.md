...
* **Only show open requests in my favourites list**: If you check here, only open requests are shown in your favourites list. This is not recommended since, if you transfer a request to another request handler (see [Ask another request handler for help][1]) and this person closes the request, it disappears from your favourites list without you being notified.
...
* **Default owner**: In this list box you select who will be the default owner of requests you create. You have three options:
  ...
  * **Unassigned**: You can also choose not to assign the request. The members of the relevant category then become responsible for dealing with the request. Such requests end up in the **Unassigned requests** lists under **Requests** (see [Use lists][2]).
