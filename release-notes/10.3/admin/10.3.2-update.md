...
## Screen designer - Assign layouts to request types
...
In the **Screen designer** for **Requests** layout, you can choose to [assign request layouts][4] based on request type instead of user group (in the **Used by** field).

1. First select assignment method (user group or request type). 

    * **User group:**, distinct request layouts are specific to the user group a person belongs to (and always the same for the user).
...
2. Then select the specific groups or types.

    ![Settings and maintenance - Screen designer Requests - Assign layout to Request types -screenshot][img3]

In this example a request type for **Invoice request** is added, and in the layout it is important to add the **Invoice number** to be shown in the **Properties sidebar** of the request ([a field is added][3] for invoice on the request table in **System design**).
