...
> [!NOTE] Because SCIM defines a small mandatory core plus a wide range of optional stuff, you will most likely find that SCIM implementations differ despite being compliant.
...
> [!NOTE] SCIM does **not** define authentication, only provisioning.
...
> [!NOTE] Because we don't have an exposed user management API, SCIM is the only way you can automate user provisioning and de-provisioning in SuperOffice.
