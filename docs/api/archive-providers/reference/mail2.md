...
|fromName| *None* |Sender name: Sender name without the e-mail address.| x |
|to|string|To: The recipient of the e-mail| x |
|toName| *None* |Recipient name: Recipient name without the e-mail address.| x |
...
```http!
GET /api/v1/archive/Mail2?$select=person/hasInfoText,person/personHasInterests,person/personUdef/SuperOffice:11,person/hasEmarketingConsent,contact/country
