...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.Archive.DocumentSelectionDynamicProviderSingleCriteriaGroup">SuperOffice.CRM.ArchiveLists.Archive.DocumentSelectionDynamicProviderSingleCriteriaGroup</see> inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/DocumentDynamicSelectionSingleCriteriaGroup?$select=person/personAddress/wgs84latitude,person/personAddress/formattedMultiLineAddress,contact/associateId,contact/email/emailBounceCount,project/projectPublish/publishedFrom
