...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.DiaryActivityArchiveProvider">SuperOffice.CRM.ArchiveLists.DiaryActivityArchiveProvider</see> inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/DiaryActivity?$select=mailMergeDraft,person/personUrl/URLAddress,contact/contactAssociate/contactDepartment,contact/saintTicketStatus,associate/contactId
