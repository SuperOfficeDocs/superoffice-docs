...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.LinksDocumentProvider">SuperOffice.CRM.ArchiveLists.LinksDocumentProvider</see> inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/LinksDocuments?$select=completed,person/restrictionAddress/state,contact/contactAssociate/middleName,project/registeredDate,project/projectAssociate/ejStatus
