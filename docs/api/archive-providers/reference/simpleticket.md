...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.SimpleTicketsProvider">SuperOffice.CRM.ArchiveLists.SimpleTicketsProvider</see> inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/SimpleTicket?$select=person/isProjectMember,person/withdrawnStoreConsent,contact/streetAddress/formattedAddress,contact/contactAssociate/userName
