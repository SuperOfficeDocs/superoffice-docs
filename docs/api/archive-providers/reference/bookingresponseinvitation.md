...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.BookingResponseInvitationProvider">SuperOffice.CRM.ArchiveLists.BookingResponseInvitationProvider</see> inside NetServer's SODatabase assembly.

This is the query provider for invitations, of subtype 'Response'

Response invitations are defined as having status declined (<see cref="F:SuperOffice.Data.AppointmentStatus.BookingDeclined" />
or <see cref="!:SuperOffice.Data.AppointmentStatus.BookingmentDeclined" />). They also have to be not overdue, i.e., activedate
...
```http!
GET /api/v1/archive/BookingResponseInvitation?$select=registeredByFullName,contact/mainContact,contact/contactAssociate/contactName,person/personNumber,person/birthMonth
