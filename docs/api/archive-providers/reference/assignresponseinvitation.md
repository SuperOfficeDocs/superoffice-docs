...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.AssignResponseInvitationProvider">SuperOffice.CRM.ArchiveLists.AssignResponseInvitationProvider</see> inside NetServer's SODatabase assembly.
...
or <see cref="F:SuperOffice.Data.AppointmentStatus.AssignmentDeclined" />). They also have to be not overdue, i.e., activedate
...
```http!
GET /api/v1/archive/AssignResponseInvitation?$select=contact/contactId,contact/contactAssociate/firstName,contact/contactAssociate/credentialDisplayValue,contact/NumberOfNotCompletedActivitiesInPeriod,contact/NumberOfNotCompletedTicketsInPeriod
