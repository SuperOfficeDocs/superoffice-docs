...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.LastTicketsProvider">SuperOffice.CRM.ArchiveLists.LastTicketsProvider</see> inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/LastTickets?$select=person/rank,person/subscription,contact/department,contact/countryId,contact/streetAddress/addressId
