...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.CancelledInvitationProvider">SuperOffice.CRM.ArchiveLists.CancelledInvitationProvider</see> inside NetServer's SODatabase assembly.

This is the query provider for invitations, of subtype 'Cancelled'

A cancelled invitation is one that has been cancelled by the owner and is not overdue.
...
```http!
GET /api/v1/archive/CancelledInvitation?$select=contact/contactAssociate/title,contact/NumberOfNotCompletedActivities,project/hasInfoText,project/projectAssociate/personId,sale/hasStakeholders
