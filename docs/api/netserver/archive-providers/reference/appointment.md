...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.AppointmentProvider">SuperOffice.CRM.ArchiveLists.AppointmentProvider</see> inside NetServer's SODatabase assembly.

Activity archive provider which handles appointments

This class instantiates a <see cref="T:SuperOffice.CRM.ArchiveLists.DiaryAppointmentProvider" /> and a <see cref="T:SuperOffice.CRM.ArchiveLists.ChecklistAppointmentProvider" /> as
...
```http!
GET /api/v1/archive/Appointment?$select=contact/LastActivity,contact/saintTicketCategory,person/firstName,person/birthMonth,person/email/emailProtocol
