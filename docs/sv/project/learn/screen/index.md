---
uid: help-sv-project-screen
title: Projektskärm
description: Projektskärmen i SuperOffice
author: SuperOffice RnD
date: 07.04.2022
keywords: projekt
topic: reference
language: sv
---

# Detaljkort på projektskärmen

Den nedre delen av projektskärmen består av detaljkort.

* [Projektguide][4] (om en sådan har definierats för den aktuella projekttypen)
* [Projektdeltagare][5]
* [Aktiviteter][6]
* [Försäljning][7]
* [Ärenden][8]

[!include[Add more section tabs](../../../learn/includes/more-tab.md)]

## Projektguide

Projektguiden som kan anpassas för att matcha dina egna unika projektprocesser. Du kan kartlägga alla aktiviteter, dokument och milstolpar i ett projekt, vilket underlättar spårningen för hela teamet. Projektguiden hjälper dig också att övervaka aktuell projektstatus och fråga vad som behöver göras.

![En projektguide hjälper teamet att ha kontinuerlig översikt över projektstatusen -screenshot][img1]

Administratören kan [lägga till och anpassa projektguider][3] åt dig.

## Vad vill du göra nu?

* [Skapa ett projekt][2]
* [Arbeta med projektguider][1]

<!-- Referenced links -->
[1]: ../project-guide/index.md
[2]: ../create.md
[3]: ../project-guide/create.md
[4]: project-guide-tab.md
[5]: project-members-tab.md
[6]: activities-project.md
[7]: sale.md
[8]: ../../../request/learn/index.md

<!-- Referenced images -->
[img1]: ../../../../media/loc/en/project/project-guide-create.png
