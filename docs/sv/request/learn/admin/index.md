---
uid: help-sv-service-settings
title: Systeminställningar
description: Systeminställningar
author: SuperOffice RnD
so.date: 06.29.2022
keywords: Tjänst
so.topic: reference
language: sv
---

# Olika inställningar för Service

I detta avsnitt kan du konfigurera inställningarna för Service, så att det passar din organisations behov. Vilka alternativ som visas och vad du kan göra beror på vilka rättigheter som har tilldelats i din användarprofil. Rättigheter tilldelas under **Användare**.

## Vad vill du veta mer om?

* [Roller][2]
* [Användare][3]
* [Kategorier][4]
* [Prioriteter][5]
* [Ärendestatus][6]
* [E-post][7]
* [Systemmallar][8]
* [Globala inställningar för Service][9]
* [Kundspråk][10]
* [Konfigurera kundcenter och källfiler][11]
* [Chatt][13]
* [SOAP][14] (endast LOKALT)
* [Systemdesign][15]
* [Profil][1] - ändringar här visas inte någonstans i SuperOffice CRM-modulen (endast i Service-modulen)

<!-- Referenced links -->
[1]: ../../../ui/blogic/learn/profiles/index.md
[2]: ../../../admin/user-management/learn/role/index.md
[3]: ../../../admin/user-management/learn/index.md
[4]: ../category/index.md
[5]: ../priority/index.md
[6]: ../status/index.md
[7]: ../../../email/service/learn/index.md
[8]: ../screen/system-templates.md
[9]: ../../../admin/preferences/learn/global-preferences/system.md
[10]: ../../../admin/options/learn/custlang/index.md
[11]: ../../../customer-center/learn/index.md
[13]: ../../../chat/learn/admin/index.md
[14]: ../../../admin/learn/screen/soap-config.md
[15]: ../../../ui/blogic/learn/index.md

<!-- Referenced images -->
