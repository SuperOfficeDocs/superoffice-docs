---
uid: help-sv-email-inbox-preview
title: fältet förhandsgranskning
description: fältet förhandsgranskning
author: SuperOffice RnD
so.date: 07.04.2022
keywords: e-post
so.topic: help
language: sv
---

# Visa e-postmeddelanden

När du markerar ett meddelande i Inkorgen förhandsvisas meddelandet till höger i fönstret.

Fliken **Förhandsgranska** innehåller följande fält:

| Fält | Beskrivning |
|---|---|
| Ämne | Här visas e-postmeddelandets ämne. |
| Från | Visar avsändaren. Om avsändaren är registrerad i SuperOffice CRM kan du hålla muspekaren över namnet för att visa kontaktkortet. |
| Datum | Här visas datum och tid för meddelandet. |
| Till | Meddelandets mottagare. |
| Bifogade filer | Här visas filer som har bifogats i meddelandet. |

> [!NOTE]
> Gå till ![ikon][img1] **Personliga inställningar** &gt; **Alternativ för e-post**. Gå till fliken **Allmänt**. Om du markerar **Visa interna bilder** visas inbyggda bilder i förhandsgranskningsfältet.

## Relaterade ämnen

* [Lägg till e-postadresser i SuperOffice CRM][1]
* [Arkivera eller öppna bilagor][2]
* [Få en förfrågan][3]
* [Skapa uppgift][4]
* [Skapa ärende][5]

<!-- Referenced links -->
[1]: manage-senders.md
[2]: archive.md#attachment
[3]: invitations.md
[4]: create-task.md
[5]: create-request.md

<!-- Referenced images -->
[img1]: ../../../../media/icons/personal-settings-small.png
