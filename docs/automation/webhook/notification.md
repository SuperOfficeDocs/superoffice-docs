...
# Mandatory fields.
title: webhook_notification       # (Required) Very important for SEO. Intent in a unique string of 43-59 chars including spaces.
description: Webhook notification  # (Required) Important for SEO. Recommended character length is 115-145 characters including spaces.
author: Tony Yates
so.date: 04.06.2018
keywords:
...
There are 3 attempts to send a webhook payload during a single cycle. If the first one fails, then the next attempt is delayed by 1 second. If the second one fails, then the third attempt is delayed by 4 seconds. If the third attempt fails, it is removed from this cycle and sent to the back of the queue. All active webhook payloads have a total of 9 consecutive attempts before the state is set to TooManyErrors(3). After that, it's up to the receiver to manage the webhook state, i.e. reset the state to Active (1), in order to begin sending webhook payloads again.

The EventID is the same for each attempt, however, if the HTTP request fails to receive a 200 response for 3 attempts, then the event is discarded, and the webhook consecutive\_errors count is incremented. When the next consecutive attempt is sent, it will be be a new EventID.

<!-- Referenced links -->
[1]: https://community.superoffice.com/documentation/SDK/SO.Database/html/Tables-appointment.htm
[2]: https://community.superoffice.com/documentation/SDK/SO.Database/html/Tables-associate.htm
[3]: https://community.superoffice.com/documentation/SDK/SO.Database/html/Tables-contact.htm
[4]: https://community.superoffice.com/documentation/SDK/SO.Database/html/Tables-person.htm
[5]: https://community.superoffice.com/documentation/SDK/SO.Database/html/Tables-project.htm
[6]: https://community.superoffice.com/documentation/SDK/SO.Database/html/Tables-projectmember.htm
[7]: https://community.superoffice.com/documentation/SDK/SO.Database/html/Tables-sale.htm
[8]: https://community.superoffice.com/documentation/SDK/SO.Database/html/Tables-SaleStakeholder.htm
