...
A trigger in SuperOffice is something that happens after a specific event (for example, person is created, sale is updated, ticket has changed status). You can create a script or a macro that runs every time this specific event happens.
