---
uid: help-no-mail-link
title: Introduksjon til SuperOffice Mail Link
description: Introduksjon til SuperOffice Mail Link
author: SuperOffice RnD
date: 06.29.2022
keywords: Mail Link
content_type: concept
language: no
---

# SuperOffice Mail Link

SuperOffice Mail Link er en plug-in som kobler e-postklienten Microsoft Outlook til SuperOffice CRM og/eller SuperOffice Service. Dette betyr blant annet at du kan:

* Arkiver e-poster og vedlegg til SuperOffice CRM.
* Arkiver en e-post som et dokument eller en oppgave i SuperOffice CRM.
* Arkiver en e-post som en sak i SuperOffice Service.
* Søke etter avsenderen av en e-post.
* Angi funksjoner for standardarkivering.
* Bruke SuperOffice-maler eller forhåndsdefinerte brødtekstmaler i e-post når du oppretter e-poster.
* Oppdater informasjonen om firma og personer i SuperOffice CRM basert på e-poster du mottar.

## Knapper på verktøylinjen i Mail Link

Disse knappene vises på verktøylinjen i Microsoft Outlook:

| Knapp | Funksjon |
|:-:|---|
| ![icon][img1] | Arkivere e-post til SuperOffice CRM |
| ![icon][img2] | Søke etter avsenderen av en e-post |
| ![icon][img3] | Klikk på den lille pilen til høyre for **SuperOffice**-knappen for å åpne dialogboksen **SuperOffice CRM-alternativer**. |

## Hva vil du gjøre nå?

* [Arkivere e-poster til SuperOffice CRM][1]
* [Opprett e-post fra SuperOffice CRM][5]
* [Opprett e-post fra Microsoft Outlook][2]
* [Søke etter avsenderen av en e-post][3]
* [Angi alternativer][4]

<!-- Referenced links -->
[1]: archive.md
[5]: create-in-superoffice.md
[2]: create-in-outlook.md
[3]: manage-senders.md
[4]: settings/index.md

<!-- Referenced images -->
[img1]: ../../../../media/icons/mail-link/archive-to-superoffice-crm.png
[img2]: ../../../../media/icons/mail-link/search-for-sender-in-superoffice-crm.png
[img3]: ../../../../media/loc/en/email/superoffice-button.png
