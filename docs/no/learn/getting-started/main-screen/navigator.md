---
uid: help-no-ui-navigator
title: Navigator
description: Navigator
author: Bergfrid Dias
date: 03.16.2023
keywords: navigatoren, meny, brukergrensesnitt
topic: reference
language: no
---

# Navigatoren

Du beveger deg mellom hovedseksjonene i SuperOffice ved hjelp av navigatoren på venstre side.

> [!TIP]
> Du kan endre bredden på navigatoren ved å klikke på ![icon][img21] og ![icon][img22] over navigatoren.

## Navigatoren i Standard CRM

| Knapp | Beskrivelse | Les mer |
|:-:|---|---|
| ![icon][img3] |Åpner **Dashboard**-skjermbildet som inneholder rapporter og diagrammer over SuperOffice-dataene dine.| [Dashbord][1] |
| ![icon][img4] | Åpner skjermbildet Firma som viser alle firmaer du er i kontakt med. | [Firmaer][2] |
| ![icon][img5] | Åpner skjermbildet Person, som inneholder alle personene dine. | [Person][3] |
| ![icon][img6] | Åpner dagbokskjermen, en slags avansert filofax. |  [Dagbok][4] |
| ![icon][img7] | Åpner skjermbildet Salg, som inneholder informasjon om alle salgene. | [Salg][5] |
| ![icon][img8] | Åpner skjermbildet Prosjekt, som inneholder informasjon om ulike prosjekter. | [Prosjekt][6] |
| ![icon][img9] | Åpner skjermbildet Utvalg, der du kan angi firmaer, personer, prosjekter, salg, dokumenter og oppfølginger som du arbeider med ofte, i separate valg. Utvalg kan blant annet brukes til utsendelser. | [Utvalg][7] |
| ![icon][img10] | Åpner **innboksen**, der du kan lese og svare på innkommende meldinger.| [Innboks/e-post][8] |
| ![icon][img11] | Åpner **Chat**-skjermen, der du kan svare på henvendelser sendt via chat-funksjonen på firmaets nettside. | [Chat][9] |
| ![icon][img12] | Åpner skjermbildet **Rapporter**. I denne rapportgeneratoren finner du et sett med standardrapporter som du kan bruke til å skrive ut rapporter basert på data fra SuperOffice CRM. | [Rapporter][10] |
| ![icon][img13] | Åpner skjermbildet **Markedsføring** . Herfra kan du opprette og sende utsendelser (e-post, SMS og dokumenter) og opprette og publisere skjemaer. | [Markedsføring][11] |
| ![icon][img14] | Åpner **Service** (eldre) | |
| ![icon][img15] | Åpner en meny som gir deg tilgang til eksterne applikasjoner og nettsteder. Elementene som er tilgjengelige for deg, er definert i Innstillinger og vedlikehold. | |

## Navigatoren i Innstillinger og vedlikehold

| Knapp | Funksjon | Les mer |
|:-:|---|---|
| ![icon][img42] | **Lisenser**: På **SuperOffice**-fanen kan du laste ned ny lisensinformasjon for din versjon av SuperOffice CRM og tilhørende funksjoner. På **Status**-fanen kan du vise databaseinformasjon og systemmeldinger. | [Lisenser][45] |
| ![icon][img43] | **Brukere**: åpner skjermbildet Brukere der du kan definere ID-er, passord, brukergrupper, roller og ulike tilgangsalternativer. På fanen **Våre firmaer** kan du legge til firmaer i tillegg til hovedfirmaet. | [Status][46] |
| ![icon][img44] | **Roller**: åpner skjermbildet Roller, der du kan definere roller for tilgang til data og funksjoner i SuperOffice CRM. Brukernes tilgangsnivåer bestemmes av rollen de tilhører. | [Roller][47] |
| ![icon][img45] | **Personvern**: åpner skjermbildet Personvern, der du kan definere innstillinger for personvern og samtykke i SuperOffice CRM. | [Personvern][48] |
| ![icon][img46] | **SAINT:**åpner skjermbildet for Statusovervåking (Sales Intelligence), der du kan opprette statuser for firmaer og prosjekter. | [SAINT][49] |
| ![icon][img47] | **AI-tjenester**: åpner skjermen for AI-tjenester hvor du kan konfigurere innstillinger for tekstanalyse, sentimentanalyse og sakskategorisering. | [AI-tjenester][20] |
| ![icon][img48] | **Lister**: åpner skjermbildet Lister, som inneholder beskrivelser og definisjoner av listene i SuperOffice CRM. | [Lister][21] |
| ![icon][img49] | **Tilbud/Sync**: åpner Quote/Sync-skjermbildet der du kan administrere tilkoblinger og innstillinger for ERP-systemer som er koblet til SuperOffice CRM. Her kan du også jobbe med prislistene og produktene som ligger i SuperOffice-databasen. | [Tilbud/Sync][22] |
| ![icon][img50] | **Arbeidsflyt**: åpner skjermbildet Arbeidsflyt der du kan definere salgsguider og prosjektveiledninger for SuperOffice CRM. | |
| ![icon][img51] | **Skjermdesigner**: åpner skjermbildet Skjermdesign der du kan opprette egendefinerte skjermlayouter for bestemte brukergrupper. | [Skjermdesigner][24] |
| ![icon][img52] | **Preferanser**: åpner skjermbildet Innstillinger der du kan angi innstillinger for ulike aspekter ved SuperOffice CRM, for eksempel antall elementer som skal vises i historikklisten, standard skrifttype for nye aktiviteter og så videre. Her kan du også konfigurere [videomøter][42] og [dokumentbibliotek][43]. | [Preferanser][25] |
| ![icon][img41] | **Chat**: åpner Chat-skjermbildet, der du kan opprette chattekanaler og konfigurere chat-funksjonen. | [Chat][2] |
| ![icon][img53] | **Alternativer**: åpner skjermbildet Alternativer der du kan definere systemomfattende innstillinger. For eksempel innstillinger for fritekstsøk, tidssoner, nummertildeling, etiketter og rapporter. | [Systemvalg][26] |
| ![icon][img54] | **Importer**: åpner skjermbildet Importer der du kan importere data til SuperOffice-databasen. | [Importer][27] |
| ![icon][img55] | **Felt**: åpner Felt-skjermen, der du definerer brukerdefinerte felt for SuperOffice CRM. | [Felt][28] |
| ![icon][img56] | **CRMScript**: Åpner skjermbildet med ulike funksjoner for skript og automatisering, for eksempel makroer, hendelseshåndtering (triggere) og planlagte oppgaver. | [CRMScript][41] |
| ![icon][img57] | **Saker**: Her finner du ulike innstillinger for saker i SuperOffice Service. | [Saker][34] |
| ![icon][img58] | **Kundesenter**: Her kan du konfigurere kundesenteret. Du kan redigere innholdet og oppsettet til kundesenteret og redigere standardinnholdet. | |
| ![icon][img59] | **Systemdesign**: her finner du forskjellige avanserte innstillinger for SuperOffice Service. | |
| ![icon][img60] | **App store**: Åpner [SuperOffice App Store][44], der du kan utforske standardapper og integrasjoner som kan utvide SuperOffice CRM Online. | |

## <a id="navigator"></a>Alternativer i navigatoren

Når du klikker på en navigatorknapp, vises en eller flere av knappene nedenfor.

| Knapp | Beskrivelse |
|:-:|---|
| ![icon][img26] | **Handlinger/lister**: Klikk på denne knappen for å vise tilgjengelige handlinger/funksjoner, lister og lignende for den valgte navigatorknappen. |
| ![icon][img23] | **Historikk**: Klikk på denne knappen for å vise nylig åpnede oppføringer, f.eks. saker, personer eller utvalg. |
| ![icon][img24] | **Favoritter**: Klikk denne knappen for å vise postene du har [angitt som favoritter][15]. |
| ![icon][img25] | **Søk**: Klikk på denne knappen for å vise resultatene av det siste søket. Søkefeltet over knappene er dynamisk - begynn å skrive inn søkefrasen for å starte søket. |

> [!NOTE]
> Ved å klikke på **Dashboard**-knappen åpnes bare dashbordet.

Hvis du holder musepekeren over en oppføring i noen av listene, vises en infoboks som inneholder tilleggsdata om oppføringen.

![Eksempel -screenshot][img2]

<!-- Referenced links -->
[1]: ../../../dashboard/learn/index.md
[2]: ../../../company/learn/index.md
[3]: ../../../contact/learn/index.md
[4]: ../../../diary/learn/index.md
[5]: ../../../sale/learn/index.md
[6]: ../../../project/learn/index.md
[7]: ../../../search-options/selection/learn/index.md
[8]: ../../../email/inbox/learn/index.md
[9]: ../../../chat/learn/index.md
[10]: ../../../reports/learn/index.md
[11]: ../../../marketing/learn/index.md
[15]: ../../basics/fav.md
[34]: ../../../request/learn/index.md

[20]: ../../../ai/learn/index.md
[21]: ../../../admin/lists/learn/index.md
[22]: ../../../quote/learn/admin/index.md
[24]: ../../../ui/screen-designer/learn/index.md
[25]: ../../../admin/preferences/learn/index.md
[26]: ../../../admin/options/learn/index.md
[27]: ../../../admin/import/learn/index.md
[28]: ../../../custom-objects/learn/udef.md

[41]: ../../../automation/crmscript/learn/index.md
[42]: ../../../admin/preferences/learn/video-meetings/index.md
[43]: ../../../admin/preferences/learn/document-library/index.md
[44]: https://online.superoffice.com/appstore
[45]: ../../../admin/license/learn/index.md
[46]: ../../../admin/user-management/learn/index.md
[47]: ../../../admin/user-management/learn/role/index.md
[48]: ../../../security/privacy/learn/index.md
[49]: ../../../saint/learn/index.md

<!-- Referenced images -->
[img2]: ../../../../media/loc/en/learn/navigator-preview-sales.png
[img3]: ../../../../../common/icons/nav-dashboard-h32.png
[img4]: ../../../../../common/icons/nav-company-h32.png
[img5]: ../../../../../common/icons/nav-contact-h32.png
[img6]: ../../../../../common/icons/nav-diary-h32.png
[img7]: ../../../../../common/icons/nav-sale-h32.png
[img8]: ../../../../../common/icons/nav-project-h32.png
[img9]: ../../../../../common/icons/nav-selection-h32.png
[img10]: ../../../../../common/icons/nav-inbox-h32.png
[img11]: ../../../../../common/icons/nav-chat-h32.png
[img12]: ../../../../../common/icons/nav-reports-h32.png
[img13]: ../../../../../common/icons/nav-marketing-h32.png
[img14]: ../../../../../common/icons/nav-cs-h32.png
[img15]: ../../../../../common/icons/nav-tools-h32.png

[img21]: ../../../../../common/icons/left-collapse.png
[img22]: ../../../../../common/icons/left-expand.png
[img23]: ../../../../../common/icons/nav-history-h32.png
[img24]: ../../../../../common/icons/nav-fav-h32.png
[img25]: ../../../../../common/icons/nav-search-h32.png
[img26]: ../../../../../common/icons/nav-list-h32.png

[img41]: ../../../../../common/icons/nav-admin-chat-active-h32.png
[img42]: ../../../../../common/icons/nav-admin-licenses-active-h32.png
[img43]: ../../../../../common/icons/nav-admin-users-active-h32.png
[img44]: ../../../../../common/icons/nav-admin-roles-active-h32.png
[img45]: ../../../../../common/icons/nav-admin-privacy-active-h32.png
[img46]: ../../../../../common/icons/nav-admin-saint-active-h32.png
[img47]: ../../../../../common/icons/nav-ai-services-active-h32.png
[img48]: ../../../../../common/icons/nav-admin-lists-active-h32.png
[img49]: ../../../../../common/icons/nav-admin-products-active-h32.png
[img50]: ../../../../../common/icons/nav-admin-workflow-active-h32.png
[img51]: ../../../../../common/icons/nav-admin-confscreen-active-h32.png
[img52]: ../../../../../common/icons/nav-admin-preferences-active-h32.png
[img53]: ../../../../../common/icons/pref-system-active-h32.png
[img54]: ../../../../../common/icons/nav-admin-import-active-h32.png
[img55]: ../../../../../common/icons/nav-admin-fields-active-h32.png
[img56]: ../../../../../common/icons/nav-admin-crmscript-active-h32.png
[img57]: ../../../../../common/icons/nav-admin-requests-active-h32.png
[img58]: ../../../../../common/icons/nav-admin-custcenter-active-h32.png
[img59]: ../../../../../common/icons/nav-admin-systemdesign-active-h32.png
[img60]: ../../../../../common/icons/nav-admin-appstore-active-h32.png
