---
uid: help-no-project-create
title: Opprette et prosjekt
description: Slik oppretter du et prosjekt og blir enda mer organisert i arbeidet.
author: SuperOffice RnD
date: 02.10.2023
keywords: prosjekt
topic: howto
language: no
---

# Opprette et prosjekt

Se denne videoen, eller følg trinnene nedenfor for å lære hvordan du oppretter et prosjekt (videolengde – 2:49):

<!-- markdownlint-disable-next-line MD034 DOCSMD007 -->
> [!Video https://www.youtube.com/embed/zLJwNll223w]

## Trinn

1. Klikk på **Ny** i toppraden, og velg **Prosjekt**.

2. I det tomme prosjektkortet fyller du ut ønskede opplysninger i feltene i fanen **Prosjekt**. Se detaljer nedenfor.

3. Alternativt kan du gå til en av fanene for å legge inn ytterligere informasjon.

4. Klikk på **Lagre**. Nå kan du [legge til prosjektdeltakere][5].

### Hva skal stå i de ulike feltene?

[!include[Steps to populate company fields](includes/project-fields.md)]

## Hva vil du gjøre nå?

* [Legge til prosjektdeltakere][5]
* [Redigere prosjekter][7]
* [Slå sammen prosjekter][8]
* [Slette prosjekter][9]
* [Arbeide med prosjektguider][1]
* [Aktiviteter][3]
* [Se hva kundene vi har i ulike bransjer bruker Prosjekt til][2] – blogg

<!-- Referenced links -->
[5]: project-members/add.md
[7]: edit.md
[8]: merge-projects.md
[9]: delete.md
[1]: project-guide/working-with.md
[2]: https://www.superoffice.com/blog/guest-blog-use-your-crm-to-manage-projects-for-all-industries/
[3]: ../../learn/basics/activity.md

<!-- Referenced images -->
