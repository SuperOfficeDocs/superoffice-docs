---
uid: help-da-gmail-link-archive-incoming
title: Arkivering af indgående e-mail
description: Arkivering af indgående e-mail
author: SuperOffice RnD
date: 02.10.2023
keywords: Gmail Link, e-mail
content_type: howto
language: da
---

# Arkivering af indgående e-mail

Du kan arkivere en indgående e-mail (med eventuelle vedhæftede filer) som et dokument i SuperOffice CRM. Dette er en praktisk metode til lagring af oplysninger, som du har modtaget via e-mail sammen med andre oplysninger om firmaer/personer i SuperOffice CRM.

![Når du har SuperOffice Gmail Link installeret, kan du arkivere e-mail, oprette aktivitet eller anmode om -screenshot][img2]

## Trin

1. Vælg den e-mail, du vil arkivere, i Gmail-indbakken.

2. Klik på ![ikonet][img1] i e-mail-værktøjslinjen.

    Hvis afsenderen af e-mailen ikke er på listen i SuperOffice CRM, vises der en advarsel øverst i dialogboksen. Der vises også en advarsel, hvis afsenderen er registreret på flere firmaer i SuperOffice CRM.

3. I dialogboksen er felterne **Firma** og **Person** udfyldt, hvis afsenderen er registreret i SuperOffice. Udfyld den relevante information i følgende felter:

    [!include[Archiving options for Gmail](includes/gmail-archiving-options.md)]

4. Klik på **Gem**. Den valgte e-mail (med eventuelle vedhæftede filer) arkiveres i SuperOffice CRM som et nyt dokument. Derefter kan du dobbeltklikke på e-mailen i detaljekortet **Aktiviteter** for at åbne den.

Klik på linket i popup-meddelelsen for at åbne dokumentet i SuperOffice.

> [!NOTE]
> Du kan også gemme vedhæftede filer som dokumenter.

## Relateret indhold

* [Arkiver vedhæftede filer][3]
* [Oprettelse af aktivitet][5]
* [Opret sag][6]
* [Administrer afsendere][1]

<!-- Referenced links -->
[1]: manage-senders.md
[3]: archive-attachments.md
[5]: archive-emails-as-activities.md
[6]: archive-emails-as-requests.md

<!-- Referenced images -->
[img1]: ../../../../../common/icons/soarchive.png
[img2]: ../../../../media/loc/en/email/email-gmaillink-archive.png
