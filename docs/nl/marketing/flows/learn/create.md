...
1. Klik op de knop **Flow** onder **Nieuw maken** aan de rechterkant van het scherm.
...
1. [Definieer flow-starttrigger en filter.](#trigger)
...
| Instelling | Beschrijving |
|---|---|
| Abonnementstype | Ingesteld op flow-niveau (in plaats van individueel op elke e-mailstap). Zorgt ervoor dat alle e-mails binnen een flow hetzelfde abonnementstype hebben. |
| Van, naam van, adres van | Dezelfde afzender op alle e-mailstappen binnen de flow. Selecteer in de lijst **Van** **Altijd gebruiken** en voer het e-mailadres van de afzender in het onderstaande veld in om dezelfde afzender voor alle mailings te gebruiken. U kunt ook een verkoop- of ondersteuningscontact als afzender kiezen, indien deze voor deze ontvanger is gedefinieerd. Voer een e-mailadres in dat moet worden gebruikt als er geen verkoop- of ondersteuningscontact beschikbaar is. Als uw beheerder de [globale e-mailvalidatievoorkeur][12] heeft geactiveerd, selecteer dan het domein waarvan u wilt verzenden in de keuzelijst. |
| Antwoord aan | Dezelfde opties als hierboven. Daarnaast kunt u **Gebruik "Van" als antwoordadres** selecteren. Alle antwoorden worden dan naar de afzender gestuurd. |
| Alleen e-mails en SMS verzenden binnen een specifiek tijdframe | Beperkt het verzendtijdframe. Deelnemers wachten in deze stap tot het toegestane tijdframe begint. Kies tussen Werkdag (ma-vr) en Weekdag (ma-zo). Specificeer tijd. Vereist dat tijdzone is ingesteld. |
| Gebruik Google Analytics | Schakelt Google Analytics-tracking in voor alle e-mails in een flow. Wanneer ingeschakeld, kunt u bron en campagne instellen. |

> [!TIP]
> Timing beïnvloedt de openings- en interactiesnelheid van e-mails. Gebruik de instelling **tijdframe** om de interactie van lezers te maximaliseren en om uw abonnees niet 's nachts te storen.
...
1. Selecteer optioneel **Deelnemer wordt direct naar de Voltooien stap verplaatst wanneer aan de succescriteria is voldaan**.
...
## <a id="trigger" />Definieer flow-starttrigger en filters
...
1. Klik op de **Trigger**-stap in het stroomdiagram.
...
1. Selecteer optioneel **Een contactpersoon kan deze flow slechts één keer starten**. Als een flow een tweede keer wordt geactiveerd op hetzelfde persoon, zorgt deze instelling ervoor dat ze niet opnieuw worden ingeschreven.

![Trigger toevoegen voor nieuwe flow -screenshot][img5]

| Trigger | Beschrijving |
|---|---|
| Persoon aangemaakt | Wanneer een nieuw persoon wordt geregistreerd, onafhankelijk van de bron (behalve import). |
| Persoon bijgewerkt | Wanneer een persoon wordt bijgewerkt, ongeacht wat er wordt bijgewerkt (behalve import en bulk update). |
| Verkoop aangemaakt | Wanneer een verkoop wordt geregistreerd op het persoon. |
...
| Verzoek aangemaakt | Wanneer een verzoek is aangemaakt op het persoon (onafhankelijk van de bron). |
...
| Link geklikt | Wanneer een persoon op een getraceerde link klikt. |

**Opmerking:**

* Als u *formulier ingediend* of *link geklikt* selecteert, let dan op eventuele waarschuwingsiconen. Formulieren en getraceerde links kunnen ook bijbehorende acties hebben.
...
1. [Selecteer waarden voor het criterium][13] in de verschillende velden op de regel (vergelijkbaar met in het Zoek-scherm).
1. Klik op **Toevoegen** om aanvullende criteria in te stellen (voegt een nieuwe regel toe).

![Contactfilter toevoegen bij trigger van nieuwe flow -screenshot][img6]
...
1. Selecteer **Stappen** in het linkermenu.
1. Kies een stap en sleep deze naar de gewenste plaats in het stroomdiagram.
...
* Een stroomdiagram met een **Trigger**, een **placeholder** voor het toevoegen van een stap en een **Voltooi**-stap (in het midden).
* Een lijst met beschikbare staptatypes (aan de rechterkant).
...
Zonder flow control zouden individuen van de ene actie naar de andere gaan totdat ze ofwel uitvallen of het einde van de flow bereiken. Ze zouden bijvoorbeeld het hele onboarding-programma in één dag kunnen ontvangen of alle nieuwsbrieven tegelijkertijd kunnen ontvangen.

Bovendien zou zonder flow control iedereen die is ingeschreven dezelfde reeks stappen volgen. Hoewel dit geschikt kan zijn voor een eenvoudige flow, kunt u door voorwaarden in te stellen voor daaropvolgende acties, bijvoorbeeld, een nieuwsbrief sturen in de voorkeurstaal van het persoon of een feedback-enquête sturen alleen naar degenen die het webinar hebben bijgewoond.
...
1. Sleep het vak **Wachttijd** uit het stappenmenu en laat het vallen in een beschikbare plaats in het stroomdiagram.
...
Actieve flow-deelnemers blijven bij deze stap voordat ze doorgaan naar de volgende stap volgens de gedefinieerde regel.
...
1. Sleep het vak **Wachten op actie** uit het stappenmenu en laat het vallen in een beschikbare plaats in het stroomdiagram.
...
5. Selecteer optioneel **Verlaat flow als er geen acties binnen de maximale wachttijd zijn**. Deelnemers die niet reageren, verlaten de flow met de status *Uitgevallen*. U kunt ervoor kiezen om ze naar een andere flow te verplaatsen en/of toe te voegen aan een statische selectie.
...
Na het ontwerpen van een flow en het opslaan ervan, sluit de **Flow-editor** en wordt het stroomdiagram weergegeven in het scherm **Flow weergeven**. De nieuwe flow heeft de status **Niet actief** totdat een flowbeheerder deze handmatig start door de schakelaar Actief in te schakelen.

## Gerelateerde inhoud

* [Stap verplaatsen, dupliceren of verwijderen][2]
* [Definieer flow-acties][3]
* [Flow splitsen][1]
* [Flow starten][6]
