---
uid: help-nl-gmail-link
title: SuperOffice Gmail Link
description: De SuperOffice Gmail Link-app verbindt uw Gmail-account met SuperOffice CRM en helpt u productiever te werken met e-mails.
author: SuperOffice RnD
date: 02.14.2023
keywords: Gmail Link, e-mail
topic: concept
language: nl
---

# SuperOffice Gmail Link

SuperOffice Gmail Link is een plug-in waarmee de client van Google Gmail wordt gekoppeld met SuperOffice. Dit betekent onder andere dat u het volgende kunt:

* Binnenkomende en uitgaande e-mails en bijlagen van uw Gmail-account archiveren als documenten in SuperOffice.
* Agenda-activiteiten maken in SuperOffice op basis van e-mails in Gmail.
* Verzoeken op basis van e-mails in Gmail.
* Afzenders opzoeken in SuperOffice: eenvoudig te volgen wizards die u helpen bij het maken van een nieuw bedrijf, contactpersoon of e-mailadres in uw SuperOffice CRM-database, als de afzender niet onbekend is.

## Gerelateerde inhoud

* [SuperOffice Gmail Link inschakelen][1]
* [Binnenkomende e-mail archiveren][3]
* [Uitgaande e-mail archiveren][4]
* [Bijlagen archiveren][2]
* [Activiteit maken][5]
* [Verzoek maken][6]
* [Afzender opzoeken in SuperOffice][7]
* [Instellingen Gmail Link bewerken][8]

<!-- Referenced links -->
[1]: install.md
[2]: archive-attachments.md
[3]: email-archive-incoming.md
[4]: email-archive-outgoing.md
[5]: archive-emails-as-activities.md
[6]: archive-emails-as-requests.md
[7]: manage-senders.md
[8]: settings.md

<!-- Referenced images -->
