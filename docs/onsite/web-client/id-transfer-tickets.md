...
# Mandatory fields.
title: id_transfer_tickets       # (Required) Very important for SEO. Intent in a unique string of 43-59 chars including spaces.
...
Enter the **ticket**, a string that proves that you are currently authenticated, without revealing who you are or how you got there. Like a paper ticket, it is only valid if it has a "stub". The stub is in the database and is put there when the ticket is issued. A ticket itself does not contain a user ID. If you can see the database then you still can't falsify valid tickets. A ticket cannot be deduced from the database. If you can write to the database... But if you can do that then you already have all the access you need.

A ticket does not contain the users' password or anything else that is intelligible - it's just a ticket. It's base-64 encoded. If you unpack it, you will see something like {3F2504E0-4F89-11D3-9A0C-0305E82C3301};1254895 - and that is all. Essentially we have two random numbers - one is very large (the [GUID][1]), the other is smaller.
...
If you want information from a ticket, give it to NetServer for authentication. If you succeed, you know who you are and your request will proceed to execute.
