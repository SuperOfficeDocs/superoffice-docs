...
Create a tablespace with a descriptive name etc. SuperOffice\_DATA and create a datafile called SUPEROFF.ORA (or some other descriptive name). The size should be 200 Mb, and we recommend that you turn on the autoextend options.
...
| Default tablespace | SuperOffice\_DATA |
...
> [!NOTE]
> Db\_block\_size can only be set when the database is created.

### Table spaces

Temporary tablespace is hard-coded to be: TEMPORARY\_DATA
