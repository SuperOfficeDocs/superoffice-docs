...
Before SuperOffice 8.1 and the [continuous database][2] this file would also have some sections that where always loaded after a fresh install, when creating a new database. These are not available from the dropdown menu inside [DBSetup.exe](https://community.superoffice.com/en/technical/documentation/administration/dbsetup/) to avoid running them unintentionally.
...
UNIQEPREFIX\_A=c:\\program files (x86)\\SuperOffice\\SuperOffice Server\\Init\\MyImportFile.imp
...
If you need to reuse a privacy key later, in another table you may use variables like #MyTableEntry\_id1, #MyTableEntry\_id2 and so on. Then in a later table in the same imp-file, you may pick up the value and assign it as a foreignKey.
