...
If the two clients are not located on the same webserver, then [Reverse proxy][1] must be set up to redirect the user to the correct location, and the reason for two sub folders is that it is easier to set up reverse proxy (ARR rules) if the clients are located in different folders.
