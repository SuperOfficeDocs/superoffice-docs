...
* [ModuleLicense][2] – one row per license
  ...
  * Hidden licenses (like the `user` login right, or the `web` webclient license) do not show in the UI, but are
...
For example: ModuleLicense `saint` is present if the system has Sales Intelligence enabled. The license is hidden, since it 
is not on the pricelist. It is a feature enabled implicitly, so it is hidden.
...
Site licenses (ModuleLicense.Type=2) are not used as much any more. In the olden days, when we have satellites, some licenses 
could be assigned to sites, rather than being available everywhere.

## User Licenses

User licenses (ModuleLicense.Type=3) are licenses that can be assigned to users.
The number of assignments can not exceed the number of licenses available.
...
User-plans have the ModuleLicense.ExtraFlags = 1 - they also define the implied licenses in the ExtraInfo field: 
`"set=user,web,chat-cal"` tells the license system to also assign the `user`, `web`, and `chat-cal` licenses when the user-plan
...
Users must have both `user` and `web` to log in to the SuperOffice web application.
This number is the upper-bound. It does not tell you how many are in use.
For some customers, the number of licenses is huge, because they are paying by use, using SCIM.

To handle this, count the number of user licenses in use, rather than the total number of licenses available.

### 2: Get the license and sum the number of ExtraFlags=1 licenses in use

User-plans are what the user is paying for. They define a number of implied, hidden licenses.
