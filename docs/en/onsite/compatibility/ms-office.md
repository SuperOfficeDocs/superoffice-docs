---
title: Microsoft Office versions
uid: tested_ms_office
description: Tested Microsoft Office versions
keywords: Microsoft Office, supported Office versions
author: SuperOffice Product and Engineering
date: 10.07.2025
content_type: reference
deployment: onsite
---

# Tested Microsoft Office versions

Tested Microsoft Office versions on both SuperOffice Win and Web applications.

Want to know if a newly released Office version is compatible? Check the [compatibility reports][3].

| Microsoft Office version    | 8.0, SR1, SR2 | 8.0 SR3 - 8.3 | 8.4 - 8.5 R15 | 8.5 R16 | 9.1 - 9.2 R03 | 9.2 R04 - R12| 10 | 11 |
|-----------------------------|:--:|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| Microsoft Office 2021       | ![i][img3] | ![i][img3] | ![i][img3] | ![i][img3] | ![i][img3] | ![i][img3] | ![i][img1] | ![i][img1] |
| Microsoft Office 2019       | ![i][img3] | ![i][img3] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] |
| Microsoft 365 for Business  | ![i][img3] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] |
| Microsoft Office 2016       | ![i][img3] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img3] |
| Microsoft Office 2013       | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img3] | ![i][img3] |
| Microsoft Office 2010 32bit | ![i][img1] | ![i][img1] | ![i][img1] | ![i][img3] | ![i][img1] | ![i][img3] | ![i][img3] | ![i][img3] |

> [!NOTE]
> Microsoft Office 2016 reached the end of support on October 14, 2025.
>
> Microsoft Office 2013 reached the end of support on April 11, 2023.
>
> Microsoft Office 2010 reached the end of support on October 13, 2020.

## Legend

[!include[legend](./includes/test-legend.md)]

<!--Referenced links-->
[3]: index.md

<!--Referenced icons-->
[img1]: ../../media/icons/testedyes.png
[img3]: ../../media/icons/testednotyet.png
