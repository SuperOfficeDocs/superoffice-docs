...
## Changes in NetServer logging from 10.2
...
* The location where this file will be stored is specified like before in the Product Configuration/web.config.
* If the log file will be created is also decided like before, in the Product Configuration/web.config by **Log to file** setting.
...
* With Debug level it logs a lot, do not keep it on for longer than necessary. Always advise customer to lower the debug level after sending the log file to support (set it back to Error).
* Users are not kicked out when changing the *appsettings.json* file but are kicked out during the recycle.
...
[img2]: media/Netserver_diagnostics_obsolete.png
[img3]: media/Log_to_file.png
[img4]: media/Log_levels.png
[img5]: media/Log_format.png
