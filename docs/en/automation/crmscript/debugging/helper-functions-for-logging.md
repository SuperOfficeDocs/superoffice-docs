---
uid: helper_functions_for_logging
...
The table has a few more fields which can be utilized, but it requires a helper-method that inserts the data into the table, using the SearchEngine:
...
To have the helper-method re-useable it should be added as its own `CRMScript`.
...
To watch the contents of the log, go to **System design/Debug log**, or (for onsite) look in the database with a SQL statement:
