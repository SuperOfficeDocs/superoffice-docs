---
uid: help-en-webtools-install
title: Install WebTools for Windows
description: If you are using Windows, you need WebTools to integrate your Outlook and Microsoft 365 applications with SuperOffice CRM.
author: Bergfrid Dias
so.date: 07.17.2023
keywords: WebTools, Windows, Mail Link, Web Extensions
so.topic: howto
language: en
---

# Install WebTools for Windows

> [!NOTE]
> On Windows, WebTools consists of two components: Mail Link and Web Extensions.

## Pre-requisites

[!include[Requirements](includes/pre-req.md)]

## Steps

Watch this video or follow the guide below to learn how to install WebTools on Windows (video length - 3:00):

<!-- markdownlint-disable-next-line MD034 DOCSMD007 -->
> [!Video https://www.youtube.com/embed/PGh6An9o8mA]

### Steps

1. Go to the ![icon][img1] Main menu in the top bar.
1. Select **Download** > **SuperOffice WebTools**. The SuperOffice WebTools wizard is displayed.
1. In the wizard, select **Personal Computer** > **SuperOffice**, and click **Download** to download the installation file
1. Click **Next** and **Close** to close the WebTools wizard.
1. Click the downloaded **SuperOffice.Web.Tools.exe** file in the downloads section in your browser.
1. Click **Continue**.
1. Click **Install**.
1. If you are asked for your user name and password to install, enter them and click **Install Software**.
1. When the message "The installation was successful" is displayed, close the dialog.
1. Finally, log out of SuperOffice CRM, close the web browser window, and reopen your browser window to log in again.

With WebTools installed, you can start creating documents from inside SuperOffice.

## Related topics

* [Install WebTools for macOS][1]

<!-- Referenced links -->
[1]: install-mac.md

<!-- Referenced images -->
[img1]: ../../../media/icons/main-menu-small.png
