---
title: Project_Agent
generated: true
---

```http
/api/v1/Project/Agent
```




* [POST Agents/Project/CreateDefaultProjectEntity](v1ProjectAgent_CreateDefaultProjectEntity.md)

* [POST Agents/Project/SaveProjectEntity](v1ProjectAgent_SaveProjectEntity.md)

* [POST Agents/Project/DeleteProjectEntity](v1ProjectAgent_DeleteProjectEntity.md)

* [POST Agents/Project/CreateDefaultProjectEventEntity](v1ProjectAgent_CreateDefaultProjectEventEntity.md)

* [POST Agents/Project/SaveProjectEventEntity](v1ProjectAgent_SaveProjectEventEntity.md)

* [POST Agents/Project/DeleteProjectEventEntity](v1ProjectAgent_DeleteProjectEventEntity.md)

* [POST Agents/Project/CreateDefaultProjectMember](v1ProjectAgent_CreateDefaultProjectMember.md)

* [POST Agents/Project/SaveProjectMember](v1ProjectAgent_SaveProjectMember.md)

* [POST Agents/Project/DeleteProjectMember](v1ProjectAgent_DeleteProjectMember.md)

* [POST Agents/Project/GetProject](v1ProjectAgent_GetProject.md)

* [POST Agents/Project/GetProjectEntity](v1ProjectAgent_GetProjectEntity.md)

* [POST Agents/Project/AddProjectMembers](v1ProjectAgent_AddProjectMembers.md)

* [POST Agents/Project/DeleteProjectMembers](v1ProjectAgent_DeleteProjectMembers.md)

* [POST Agents/Project/GetProjectImage](v1ProjectAgent_GetProjectImage.md)

* [POST Agents/Project/SetProjectImage](v1ProjectAgent_SetProjectImage.md)

* [POST Agents/Project/DeleteProjectMemberByIds](v1ProjectAgent_DeleteProjectMemberByIds.md)

* [POST Agents/Project/GetDuplicates](v1ProjectAgent_GetDuplicates.md)

* [POST Agents/Project/CreateNewEntry](v1ProjectAgent_CreateNewEntry.md)

* [POST Agents/Project/SetDuplicateRulesStatus](v1ProjectAgent_SetDuplicateRulesStatus.md)

* [POST Agents/Project/GetDuplicateRules](v1ProjectAgent_GetDuplicateRules.md)

* [POST Agents/Project/Merge](v1ProjectAgent_Merge.md)

* [POST Agents/Project/IsNumberValid](v1ProjectAgent_IsNumberValid.md)

* [POST Agents/Project/HasGuide](v1ProjectAgent_HasGuide.md)

* [POST Agents/Project/GetNextMilestone](v1ProjectAgent_GetNextMilestone.md)

* [POST Agents/Project/OfferAutoNextStatusOnApppointmentCompleted](v1ProjectAgent_OfferAutoNextStatusOnApppointmentCompleted.md)

* [POST Agents/Project/GetNextProjectStatus](v1ProjectAgent_GetNextProjectStatus.md)

* [POST Agents/Project/HasGuideActivities](v1ProjectAgent_HasGuideActivities.md)

* [POST Agents/Project/ValidateProjectEntity](v1ProjectAgent_ValidateProjectEntity.md)

* [POST Agents/Project/GetProjectEvent](v1ProjectAgent_GetProjectEvent.md)

* [POST Agents/Project/GetProjectEventOnPerson](v1ProjectAgent_GetProjectEventOnPerson.md)

* [POST Agents/Project/GetProjectEventEntity](v1ProjectAgent_GetProjectEventEntity.md)

* [POST Agents/Project/GetProjectEventEntityFromProjectId](v1ProjectAgent_GetProjectEventEntityFromProjectId.md)

* [POST Agents/Project/DeleteProjectEventEntityFromProjectId](v1ProjectAgent_DeleteProjectEventEntityFromProjectId.md)

* [POST Agents/Project/GetProjectEventList](v1ProjectAgent_GetProjectEventList.md)

* [POST Agents/Project/GetMyProjectEvents](v1ProjectAgent_GetMyProjectEvents.md)

* [POST Agents/Project/GetProjectEventsOnPerson](v1ProjectAgent_GetProjectEventsOnPerson.md)

* [POST Agents/Project/GetProjectList](v1ProjectAgent_GetProjectList.md)

* [POST Agents/Project/GetMyProjects](v1ProjectAgent_GetMyProjects.md)

* [POST Agents/Project/GetMyMemberProjects](v1ProjectAgent_GetMyMemberProjects.md)

* [POST Agents/Project/GetProjectsFromPerson](v1ProjectAgent_GetProjectsFromPerson.md)

* [POST Agents/Project/GetProjectsFromContact](v1ProjectAgent_GetProjectsFromContact.md)

* [POST Agents/Project/GetPublishedProject](v1ProjectAgent_GetPublishedProject.md)

* [POST Agents/Project/GetPublishedProjects](v1ProjectAgent_GetPublishedProjects.md)

* [POST Agents/Project/GetMyPublishedProjects](v1ProjectAgent_GetMyPublishedProjects.md)

* [POST Agents/Project/GetPublishedProjectsOnPersonId](v1ProjectAgent_GetPublishedProjectsOnPersonId.md)

* [POST Agents/Project/GetProjectMember](v1ProjectAgent_GetProjectMember.md)

* [POST Agents/Project/UpdateProjectMembers](v1ProjectAgent_UpdateProjectMembers.md)

* [POST Agents/Project/GetProjectMembers](v1ProjectAgent_GetProjectMembers.md)

* [POST Agents/Project/UpdateProjectMember](v1ProjectAgent_UpdateProjectMember.md)

* [POST Agents/Project/GetProjectMembersById](v1ProjectAgent_GetProjectMembersById.md)

* [POST Agents/Project/SaveProjectMembersFunctionAndComment](v1ProjectAgent_SaveProjectMembersFunctionAndComment.md)