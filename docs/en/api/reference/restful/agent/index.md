...
The Agent API is a SuperOffice POST-only `HTTP RPC` API

## Welcome

Welcome to the SuperOffice Agents WebAPI. You can get the version number and build-date from the API endpoint.
...
* **Agents API** - the POST-only `HTTP RPC` API.
...
GET this to get a description of the call with some documentation. The method is not invoked using GET, even if the method is called `GetAppointment`.
...
The examples below are given using JavaScript-y pseudo-code.
...
```javascript
req = { 
        UdListDefinitionName: "category", IncludeDeleted: true 
      }
...
We could call `SaveDocumentTemplateEntity` and `SaveDocumentTemplateStream` separately, like the REST API does, or we can use the agent call that does both in a single call:
