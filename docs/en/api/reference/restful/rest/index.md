...
* **Agents API** - Is a POST-only `HTTP RPC` API.
...
The agents expose the latest Services agents and functions as HTTP RPC endpoints.

`/api/v1/Agents/Appointment/CalculateDays`

GET this to get a description of the call. The method is not invoked using GET, even if the method is called **GetAppointment**.
...
If the **Accept-Language: en** header is added, then we get back a parsed
result:
