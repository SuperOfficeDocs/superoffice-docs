...
namespace DevNet.Util
{
  ...
  publicsealedclassUDFHelper
  {
    ...
    /// UDFInfo that contains the value of the user defined field.
    ...
    /// UDFInfo struct that is prepopulated with Entity
    /// FieldInfo, Entity Identity, and User-Defined FieldInfo objects.
    /// When Type of a user-defined field is DropDown
      ...
      int entityId, string udFieldName)
    {
      ...
      if (info.UsrDefField != null)
      {
        ...
        obj = (object[])GetUDFData(info.EntityId, info.UsrDefField, info.EntityField);
         ...
         int entityId, string udFieldName, ListItemSource listSource)
    {
      ...
      return GetUDFInfo(entityId, udFieldName, info.EntityField,info.UsrDefField, listSource);
    }
      ...
      FieldInfo entityField, FieldInfo udField, ListItemSource listSource)
    {
      ...
      object[] obj = (object[])GetUDFData(info.EntityId, info.UsrDefField, info.EntityField);

      if (info.EntityField == null || info.UsrDefField == null)
        thrownewApplicationException("Unknown user-defined field.");

      if(obj == null)
        return info; // throw new ApplicationException("Unable to get UDF base on supplied information.");
    ...
    ///
    /// UDFInfo object that contains both Entity and User-Defined
    /// FieldInfo objects (Retrieved by first populating a UDFInfo struct by using the
    /// method. Make sure to set the FieldValue before calling this method.
    /// Return 1 if the value was updated.
    /// Returns 2 is the field was new and required an insert. Otherwise 0.
    ...
    ///
    ///The entity table that contains the user-defined field.
    ///The identity of the entity that contains the user-defined field.
    ///The field label, or name, as defined in the admin application.
    ///The new value of the user-defined field.
    ///Return 1 if the value was updated. Returns 2 is the field was new
    ///and required an insert. Otherwise 0.
    publicint SetUDFValue(EUDefType entityTable, int entityId, string udFieldName, T udFieldValue)
    {
      AssertNotNull((Nullable<>int>)entityId, "entityId");
    ...
    /// Make sure to set the FieldValue before calling this method.
    ///
    ///Must be an Integer, Long, or String type.
    ///The identity of the entity that contains the user-defined field.
    ///The user-defined FieldInfo object (for example
    ///UDContactSmall.long05, string05).
    ///The entire user-defined FieldInfo object (for example
      ...
      string udFieldName, T searchValue)
    {
      ...
      if (info.EntityField == null || info.UsrDefField == null)
        thrownewApplicationException(
          string.Format("Unable to find user-defined field: {0}, therefore search " + "for {1} can not continue.", udFieldName, searchValue));
      ...
      {
        Argument[] argArr = GetArguments(searchValue, info.UsrDefField.DataType);
    ...
    ///An integer array of entity IDs that contain the user-defined field of the specified value.
    ///Thrown when one of the argumentsare null.
    ///Thrown when a user-defined field is not found, for example udFieldName
    /// can not be found in the database.
    ///
    /// int[] a = UDFHelper.Instance.UDFSearch(EUDefType.Contact, "Start Date",
    /// new int[] { 1097107200, 1098144000 }, UDFHelper.Operation.Between);
    /// int[] j = UDFHelper.Instance.UDFSearch(EUDefType.Contact,"NewestUDField", 7);
    ///
    /// If is an Array, the default Operation is anIn(,...) operation.
    ///
    publicint[] UDFSearch(SuperOffice.HDB.Util.EUDefType entityTable,
      string udFieldName, T searchValue, Operation operation)
    {
      ...
      if (info.EntityField == null || info.UsrDefField == null)
        thrownewApplicationException(string.Format("Unable to find user-defined field: {0}, therefore search for {1} can not continue.",udFieldName, searchValue));

      //Select all rows where UDF equals searchValue
      Select select = S.NewSelect();
      select.ReturnFields.Add(info.UsrDefField.Parent.All[0]);
      select.Restriction = GetSearchRestriction(info.UsrDefField, searchValue, operation);
    ...
    ///The culturename, typically en-US, nb-NO, but also US, NO is accepted Culture specific string part
    publicstring ParseMultiLanguageString(string multilangString, string culturename)
    {
    ...
    ///
    ///SuperOffice.HDB.Util.EUDefType enum value.
    ///String representation of the user-defined table enumeration
    ...
    ///
    /// This method is used to retrieve the column if for a specific user-defined field.
    ///
    ///The entity that contains the user-defined field.
    ///The name of the user-defined field.
    ///The column identity of the supplied user-defined field.
    publicint GetUDFColumnID(SuperOffice.HDB.Util.EUDefType ownerTableId, string fieldName)
    {
    ...
    ///
    ///The identity value that corresponds to the main entity table record.
    ///FieldInfo object that is the column in the user-defined table
    /// (for example long05, string05).
    ...
    publicobject GetUDFData(int entityIdentity, FieldInfo udField, FieldInfo entityField)
    {
      AssertNotNull((Nullable<>int>)entityIdentity, "entityIdentity");
      ...
      select.JoinRestriction.InnerJoin(entityField.Equal(udField.Parent.All[0]));
      select.Restriction = entityField.Parent.All[0].Equal(S.Parameter(entityIdentity));
    ...
    ///
    ///The SODBIF Select object that is the query.
    ...
    ///then updates the entity record udef field with the UDF record identity.
    ///
    ///FieldInfo object of the user-defined field to insert a value into.
    ///The value to insert into udField.
    ///FieldInfo object, used to access the entity's TableInfo object.
    ///Entity identity, used to determine the entity record to update.
    ...
    ///
    ///The field to update. Could be Entity or UDF field.
    ///The identity of the row, or record, to update.
    ///The new field value.
    ///
    ...
    //for Nullable types
    privatestaticvoid AssertNotNull(Nullable arg, string argName) where T : struct
    {
      if (!arg.HasValue)
      {
        thrownewArgumentNullException(argName);
