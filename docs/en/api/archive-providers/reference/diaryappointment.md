...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.DiaryAppointmentProvider">SuperOffice.CRM.ArchiveLists.DiaryAppointmentProvider</see> inside NetServer's SODatabase assembly.

Archive provider for Appointment activities, of the Diary type (not followup or booking). This provider can be used on its own, or can form part of the
...
```http!
GET /api/v1/archive/DiaryAppointment?$select=appointmentPublish/isPublished,appointmentPublish/publishedFrom,contact/contactAssociate/credentialType,person/personAssociate/ejDisplayName,person/correspondingAssociate/simultaneousEjUser
