...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.ProjectMemberProvider">SuperOffice.CRM.ArchiveLists.ProjectMemberProvider</see> inside NetServer's SODatabase assembly.

Provider for the Project Member archive under Project

There are two important, optional entities: 'retired' (for retired persons) and 'stop' (contacts makred with stop).
If these are not selected through SetDesiredEntities, project members of these types will be dropped
from the result.
...
|projectMemberId|int|SR\_PM\_ID: SR\_PM\_ID\_TOOLTIP| x |
...
|eventParticipantStatus|listAny|SR\_FIELD\_PROJECT\_MEMBER\_EVENT\_STATUS: SR\_FIELD\_PROJECT\_MEMBER\_EVENT\_STATUS| x |
|function|listAny|SR\_PROJECTMEMBER\_FUNCTION: SR\_PROJECTMEMBERARCHIVE\_FUNCTION\_TOOLTIP| x |
|hasInfoText|bool|SR\_PROJECTMEMBER\_HASINFO: SR\_PROJECTMEMBERARCHIVE\_HASINFO\_TOOLTIP| x |
|registered|datetime|SR\_PROJECTMEMBER\_REGISTERED: SR\_REGISTEREDDATE\_TOOLTIP| x |
|updated|datetime|SR\_PROJECTMEMBER\_UPDATED: SR\_UPDATEDDATE\_TOOLTIP| x |
|textId|int|Text ID| x |
|infoText|positiveString|SR\_SINGULAR\_INFO: SR\_DIARYARCHIVE\_DESCRIPTION\_TOOLTIP| x |
...
```http!
GET /api/v1/archive/ProjectMember?$select=project/registeredBy,project/NumberOfNotCompletedSales,person/personAssociate/contactCategory,person/personAssociate/ejStatus
