...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.FindDocumentProvider">SuperOffice.CRM.ArchiveLists.FindDocumentProvider</see> inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/FindDocument?$select=icon,visibleFor,person/restrictionAddress/county,person/personUdef/SuperOffice:10,contact/url/URLDescription
