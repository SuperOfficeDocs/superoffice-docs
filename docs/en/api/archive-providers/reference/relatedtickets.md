...
|relationType|listAny|SR\_ADMIN\_TICKET\_RELATION\_TYPE: SR\_ADMIN\_TICKET\_RELATION\_TYPE| x |
|sourceTicketId|int|| x |
|comment|string|SR\_RELATION\_COMMENT: SR\_RELATION\_COMMENT\_TOOLTIP| x |

## Sample

```http!
GET /api/v1/archive/RelatedTickets?$select=contactId,person/restrictionAddress/state,person/personAssociate/ejStatus,contact/restrictionAddress/county,sale/time
