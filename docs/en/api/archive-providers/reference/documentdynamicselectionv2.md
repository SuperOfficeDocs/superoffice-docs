...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.Archive.DocumentSelectionDynamicProviderV2">SuperOffice.CRM.ArchiveLists.Archive.DocumentSelectionDynamicProviderV2</see> inside NetServer's SODatabase assembly.

Document selection archive with OR-able selection groups. Each group is represented with the <see cref="T:SuperOffice.CRM.ArchiveLists.Archive.DocumentSelectionDynamicProviderSingleCriteriaGroup" />.
...
```http!
GET /api/v1/archive/DocumentDynamicSelectionV2?$select=person/restrictionAddress/zip,person/restrictionAddress/formattedAddress,person/personAssociate/simultaneousEjUser,person/correspondingAssociate/assocType,contact/email/emailBounceCount
