...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.NewCategoryMembersProvider">SuperOffice.CRM.ArchiveLists.NewCategoryMembersProvider</see> inside NetServer's SODatabase assembly.

Archive of NewCategoryMember - used by the web api.
Lists new CategoryMembers added through restriction.
...
```http!
GET /api/v1/archive/NewCategoryMembers?$select=ejDisplayName,associateDbId,contactCategory
