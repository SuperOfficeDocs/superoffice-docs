...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.SimpleDocumentsProvider">SuperOffice.CRM.ArchiveLists.SimpleDocumentsProvider</see> inside NetServer's SODatabase assembly.

## Supported Entities
| Name | Description |
| ---- | ----- |
|"document"|[[SR_SINGULAR_DOCUMENT]]|
...
```http!
GET /api/v1/archive/SimpleDocument?$select=person/firstName,person/supportAssociate,person/supportAssociateFullName,person/restrictionAddress/formattedAddress,contact/NumberOfNotCompletedTicketsInPeriod
