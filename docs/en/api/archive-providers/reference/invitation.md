...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.InvitationProvider">SuperOffice.CRM.ArchiveLists.InvitationProvider</see> inside NetServer's SODatabase assembly.
...
<seealso cref="T:SuperOffice.CRM.ArchiveLists.SimpleInvitationProvider" /><seealso cref="!:ResponseInvitationProvider" /><seealso cref="T:SuperOffice.CRM.ArchiveLists.CancelledInvitationProvider" /><seealso cref="!:RepeatingInvitationProvider" /><seealso cref="T:SuperOffice.CRM.ArchiveLists.RejectedInvitationProvider" /><seealso cref="T:SuperOffice.CRM.ArchiveLists.ExplicitInvitationProvider" />
The final result set needs filtering to remove repeating records if we are running in "compressed" mode, and also reordering
to make sure we get the right order. We also need to filter repeating bookings against explicit records to remove duplicates there,
...
| Name | Description |
| ---- | ----- |
|"invitation"|[[SR_ENTITY_INVITATION]]|
|"cancelled"|[[SR_ENTITY_CANCELLED_INVITATION]]|
|"rejected"|[[SR_INVITE_SHOW_DECLINED]]|
|"response"|[[SR_INVITE_SHOW_RESPONSES]]|
|"repeating"|[[SR_INVITE_SHOW_RECURRING]]|
...
|invitationStatusIcon| *None* |SR\_INVITATION\_STATUS\_ICON\_TOOLTIP| x |
|invitationStatus| *None* |SR\_PROJECTARCHIVE\_STATUS: SR\_PROJECTARCHIVE\_STATUS| x |
...
|invitationRestrictionIds|int|SR\_RESTRICTION\_INVITATIONID: SR\_RESTRICTION\_INVITATIONID\_TOOLTIP|  |

## Sample

```http!
GET /api/v1/archive/Invitation?$select=contact/email/emailHasBounced,contact/postAddress/county,contact/postAddress/formattedAddress,contact/streetAddress/formattedMultiLineAddress,person/personCountryId
