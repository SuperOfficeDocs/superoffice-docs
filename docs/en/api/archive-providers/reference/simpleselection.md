...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.SimpleSelectionsProvider">SuperOffice.CRM.ArchiveLists.SimpleSelectionsProvider</see> inside NetServer's SODatabase assembly.
...
|selection/infoText|positiveString|SR\_LABEL\_DESCRIPTION: SR\_DIARYARCHIVE\_DESCRIPTION\_TOOLTIP| x |

## Sample

```http!
GET /api/v1/archive/SimpleSelection?$select=selectionAssociate/personId,lastMembershipChange,selectionAssociate/personId
