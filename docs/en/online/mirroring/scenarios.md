...
| Client is offline for **more than 7 days** | Events have been purged from Kafka. | The client cannot fill the gap by replay. Run [`resync-tables`](force-resync.md) on the affected tables (or, in extreme cases, request a full reset from SuperOffice Operations). |
