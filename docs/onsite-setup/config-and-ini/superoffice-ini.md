...
# Mandatory fields.
title: superoffice_ini # (Required) Very important for SEO. Intent in a unique string of 43-59 chars including spaces.
description: SuperOffice.ini # (Required) Important for SEO. Recommended character length is 115-145 characters including spaces.
...
If the other parameters do not exist, they are given values that are determined by archivepath, for example:
...
| LogPath= | Where log is saved. Defaults to SO\_ARC\\DATA |
...
| EnableSoArcLog=0/1 | Enable debug logging from SO\_Arc document plugin off/on |
