---
uid: help-de-project-screen
title: Projektansicht
description: Projektansicht in SuperOffice
author: SuperOffice RnD
so.date: 07.04.2022
keywords: Projekt
so.topic: reference
language: de
---

# Bereichsregisterkarten in der Ansicht Projekt

Der untere Teil der Projektansicht besteht aus Bereichsregisterkarten.

* [Projektleitfaden][4] (falls für den aktuellen Projekttyp definiert)
* [Projektmitglieder][5]
* [Aktivitäten][6]
* [Verkäufe][7]
* [Anfragen][8]

[!include[Add more section tabs](../../../learn/includes/more-tab.md)]

## Projektleitfaden

Projektleitfaden, der an Ihre eigenen eindeutigen Projektprozesse angepasst werden kann. Sie können alle Aktivitäten, Dokumente und Meilensteine eines Projekts zuordnen, was es Ihrem gesamten Team leicht macht, sie zu verfolgen. Der Projektleitfaden hilft Ihnen auch dabei, den aktuellen Projektstatus zu überwachen und jederzeit zu erkennen, was zu tun ist.

![Ein Projektleitfaden hilft dem Team, einen konstanten Überblick über den Projektstatus zu behalten -screenshot][img1]

Ihr Administrator kann für Sie [Projektleitfäden hinzufügen und anpassen][3].

## Was möchten Sie jetzt tun?

* [Projekt erstellen][2]
* [Mit Projektleitfäden arbeiten][1]

<!-- Referenced links -->
[1]: ../project-guide/index.md
[2]: ../create.md
[3]: ../project-guide/create.md
[4]: project-guide-tab.md
[5]: project-members-tab.md
[6]: activities-project.md
[7]: sale.md
[8]: ../../../request/learn/index.md

<!-- Referenced images -->
[img1]: ../../../../media/loc/en/project/project-guide-create.bmp
