...
| company\_name | X | X | The current user's company name. |
...
| is\_administrator | X | X | Determine whether the current user is an administrator. |
| lastname | X | | The current user's last name. |
| netserver\_url | X | X | The URL to a tenant SOAP web service.<br>Often used in conjunction with SuperOffice [.NET NuGet proxies][1].<br>New applications should always use the latest. |
| remember\_me\_expires | X | X | Unused. |
| serial | X | X | The tenant database serial number. |
| so\_primary\_email\_address | X | X | The current user's primary email address. (added June 2019) |
| system\_token | X | X | A unique identifier used to exchange for a system ticket.<br>Used for background processing, back-channel communications. |
| ticket | X | | A current user's unique identifier, used for authentication. |
| upn | X | X | Specifies a user principal name (UPN). |
| webapi\_url | X | X | The URL to a tenant REST web services. |
